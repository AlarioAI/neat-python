...
class Node:
    def __init__(self, label, activation:Callable, state:float=0.):
        self._label = label
        self._state = state
...
#        print(f"state = {self._state}, id={self.label}")
...
class Graph:
    def __init__(self, adjacency_dict, output_node_ids:list[int | str], max_steps:int=6):
    ...
    def genes_to_adjacency(genome, config):
        ...
        return adjacency_matrix, config.genome_config.output_keys
